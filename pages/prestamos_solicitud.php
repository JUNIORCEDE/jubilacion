...
$CantidadMostrar = 20;
$cont = 0;
$consultavistas ="SELECT * FROM prestamos_solicitud ORDER BY prestamos_solicitud.fecha ASC LIMIT ".(($compag-1)*$CantidadMostrar)." , ".$CantidadMostrar;
if (!is_null($cedula) and !($cedula=="") and ($cedula!=0)){
    $consulta = $model->Listar("SELECT * FROM prestamos_solicitud where cedula = ".$cedula);
    $cont = count($model->result);
    $TotalRegistro = ceil($cont/$CantidadMostrar);
    $consultavistas ="SELECT * FROM prestamos_solicitud where cedula = ".$cedula." ORDER BY prestamos_solicitud.fecha ASC LIMIT ".(($compag-1)*$CantidadMostrar)." , ".$CantidadMostrar;
}
else{
    $model->Listar("SELECT * FROM prestamos_solicitud");
    $cont = count($model->result);
    $TotalRegistro = ceil($cont/$CantidadMostrar);
}
if($cont<1){
